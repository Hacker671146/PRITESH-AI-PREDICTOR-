...
function checkPreviousPrediction(period, actualNumber) {
    ...
    console.log(`[CHECK] Period ${period} | Pred: ${predictedCategory} | Actual: ${actualCategory} (${actualNumber}) → ${isWin ? "WIN ✅" : "LOSS ❌"} | Conf: ${predObj.confidence}`);
...
async function executeBet(period, prediction, confidence) {
    ...
    console.log(`[EXECUTE BET] Period: ${period}, Prediction: ${prediction}, Confidence: ${confidence}, Bet Amount: $${betAmount}`);
    console.log(`[EXECUTE BET] Potential Win: $${winAmount.toFixed(2)}`);
    
    // In production, this would call your betting platform API
    // For demonstration, we just return a simulated bet response
    return {
        success: true,
        betId: `BET-${Date.now()}`,
        period: period,
...
(async function start() {
    ...
    // Run every minute (adjust based on actual game interval)
    setInterval(async () => {
        await update();
    }, 60000);
})();
...
app.get('/', (req, res) => {
    res.json({ 
        status: "active", 
        name: NAME, 
        version: "3.0 - Advanced Ensemble AI with Bet Flow",
